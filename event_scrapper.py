...
#                 # Split date and event
#                 parts = event_text.split(": ", 1)
#                 if len(parts) == 2:
#                     date, event = parts
#                 else:
#                     date, event = "Unknown date", parts[0]
#                 events.append(
#                     {"Year": current_year, "Date": date.strip(), "Event": event.strip()}
#                 )

# # Display results
# if events:
#     for event in events:
#         print(f"Year: {event['Year']}, Date: {event['Date']}, Event: {event['Event']}")
# else:
#     print("No events found. Check the script or page structure.")
